...
if [[ "${EUID:-$(id -u)}" -ne 0 && "${POCKET4_NO_SUDO:-0}" != "1" ]]; then
  echo "MPP decode usually requires root on BELABOX; re-running with sudo..."
  exec sudo -E bash "$0" "$@"
fi

RTMP_URL="${RTMP_URL:-rtmp://127.0.0.1:1935/publish/live}"
PLUGIN_DIR="/opt/belabox-pocket4-rtmp-hevc"
REGISTRY_FILE="/tmp/gst-registry-pocket4-live-decode.bin"

echo "Testing live decode from: $RTMP_URL"
...
gst-launch-1.0 -v \
  ...
  mppvideodec ! fakesink sync=false
